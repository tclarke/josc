...
var EPOCH_ADJUST = 2208988800; // number of seconds from January 1, 1900 to January 1, 1970
var FP_ADJUST = 4295.0; // adjust between decimal and binary fixed point
...
function OSCPacket(arg) {
   ...
   if (arg instanceof OSCMessage || arg instanceof OSCBundle) {
      ...
      for (i in arg) {
         if (arg instanceof OSCMessage || arg instanceof OSCBundle) {
            ...
            throw "Argument must be an OSCMessage, OSCBundle or an array.";
         }
      }
   } else if (arg === undefined) {
      // do nothing
...
OSCPacket.prototype = {
   ...
   sendDatagrams: function() {
      for (i in this.packets) {
         return this.packets[i].toBuffer().toString('binary');
      }
   },
   recvFromStream: function(data) {
      ...
      while (offset < b.length) {
         ...
         b.copy(tmp, 0, offset, offset + sz); offset += sz;
         if (OSCBundle.isBundle(tmp)) {
            this.packets.push(new OSCBundle(tmp));
         } else {
            this.packets.push(new OSCMessage(tmp));
         }
      }
   },
   recvDatagram: function(data) {
      var b = new Buffer(data, 'binary');
      if (OSCBundle.isBundle(b)) {
         this.packets.push(new OSCBundle(b));
      } else {
         this.packets.push(new OSCMessage(b));
      }
   }
}

var dgram = require('dgram');
function UDPServer(port, addr) {
   this.port = port;
   this.addr = addr;
   this._socket = dgram.createSocket('udp4');
   this._socket._container = this;
   this._socket.on("message", function(msg) {this._container._recv(msg)});
}

UDPServer.prototype = {
   start: function() { this._socket.bind(this.port, this.addr); },
   close: function() { this._socket.close(); },
   on: function(pat, callable) {
      if (!(pat in self._callbacks)) {
         self._callbacks[pat] = new Array();
      }
      this._callbacks[pat].push(callable);
   },
   ...
   _dispatch: function(pkt) {
      console.log("dispatch: " + pkt)
   }
}
